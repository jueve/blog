...
const objects = {
  src: `${imageDir}/relation-between-objects.png`,
  alt: "function objectとconstructorの関係",
  caption: "function objectとconstructorの関係",
};
const tree = {
...
const runtime01 = {
  src: `${imageDir}/runtime01.png`,
  alt: "runtime semanticsにおける変数と引数の対応",
  caption: "runtime semanticsにおける変数と引数の対応",
};
const runtime02 = {
  src: `${imageDir}/runtime02.png`,
  alt: "runtime semanticsにおける変数と引数の対応",
  caption: "runtime semanticsにおける変数と引数の対応",
};
...
> オブジェクトを生成し、`new`演算子か`super`の宣言を経由して呼び出される。第一引数にconstructorの呼び出しの引数を含むリスト、または`super`を使った宣言の引数を含むリストを与える。第二引数には`new`演算子が最初に適用されたオブジェクトを与える。このinternal methodを実装したオブジェクトはconstructorと呼ばれる。function objectはconstructorである必要はなく、constructorではないfunction objectは[[Constructor]]というinternal methodを持たない。(拙訳)
...
<ImageWithCaption {...runtime02} />

さらに今一度、表記編で扱った画像を確認したい。

<ImageWithCaption {... tree} />

つまり`ref`は`Vehicle`の評価の結果、`function`で宣言した`Vehicle`の参照の結果を意味する。言い換えると、`EvaluateNew`内で生成される変数`constructor`の源は`function Vehicle`の参照の結果である。
