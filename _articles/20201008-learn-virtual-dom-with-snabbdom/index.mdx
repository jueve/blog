...
const vnodeStep1 = {
  src: `${imageDir}/vnode-step01.png`,
  caption: 'div要素の確認。',
  alt: 'div要素の確認。',
};
const vnodeStep2 = {
  src: `${imageDir}/vnode-step02.png`,
  caption: 'p要素の確認。',
  alt: 'p要素の確認。',
};
const vnodeStep3 = {
  src: `${imageDir}/vnode-step03.png`,
  caption: 'a要素の確認。AliceからBobにテキストを変更する。',
  alt: 'a要素の確認。AliceからBobにテキストを変更する。',
};
const vnodeStep4 = {
  src: `${imageDir}/vnode-step04.png`,
  caption: 'ul要素の確認。',
  alt: 'ul要素の確認。',
};
const vnodeStep5 = {
  src: `${imageDir}/vnode-step05.png`,
  caption: 'li要素1つ目。',
  alt: 'ul要素1つ目。',
};
const vnodeStep6 = {
  src: `${imageDir}/vnode-step06.png`,
  caption: 'li要素2つ目。',
  alt: 'li要素2つ目。',
};
const vnodeStep7 = {
  src: `${imageDir}/vnode-step07.png`,
  caption: 'li要素3つ目。変更前には存在しないノードが変更後にあることは追加を意味する。',
  alt: 'li要素3つ目。変更前には存在しないノードが変更後にあることは追加を意味する。',
};
const vnodeStep8 = {
  src: `${imageDir}/vnode-step08.png`,
  caption: '差分検知アルゴリズムが完了した状態。',
  alt: '差分検知アルゴリズムが完了した状態。',
};
...
**仮想DOM自身が速いと表現するよりは、仮想DOMが最小限の回数でreal DOM nodeの更新をしてくれるため、コードを書く人間が闇雲にDOM APIを使ってreal DOM nodeを更新してしまうよりも相対的に速くなるという認識が正しいように思う。**
...
`VNode`のプロパティのうち、仮想DOMの差分検知アルゴリズムの理解に最低限必要なのは`sel`、`children`、`elm`、`text`、`key`になる。
...
### toVNode関数
`VNode`の生成には`toVNode`関数もある。これはDOMを直接`VNode`に変換する関数になる。
...
```typescript
// `init`関数の第一引数にはhookに利用したいモジュールを配列で渡す。
// 空の配列を渡した場合、hookが動かずに仮想DOMの差分検知アルゴリズムだけが動作する。
...
const vNode1ToVNode2 = patch(temp, vnode2);
const divToVNode1 = temp.elm;
const vNode1ToVNode2 = result.elm;
...
<ImageWithCaption {...vnodeStep3} />

左側のノードは終端まで見たので、今度はdivの子要素であるulに移る。変更が無いので、子要素を見る。
<ImageWithCaption {...vnodeStep4} />

li要素1つ目。変更はないので次に移る。
<ImageWithCaption {...vnodeStep5} />

li要素2つ目。変更はないので次に移る。
<ImageWithCaption {...vnodeStep6} />
...
<ImageWithCaption {...vnodeStep7} />

これにより差分の反映が終了する。
<ImageWithCaption {...vnodeStep8} />
...
### hooksの実行
hooksにはモジュール経由で使用できるものとDOM APIで実DOMを操作する際に使われるものがある。
各hookには実行タイミングみたいなものがあり、これはhookの実行をどの段階で行うかという基準で分類されている。
